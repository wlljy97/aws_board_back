...
//    @Pointcut("execution(* com.korit.board.controller.*.*(..))") // 모든영역에 쓰임
    @Pointcut("@annotation(com.korit.board.aop.annotation.ValidAop)") // pointcut에서 오타가 나면 실행이 되지 않음
    private void pointCut() {}

    @Around("pointCut()") /* 포인트컷 */
    public Object around(ProceedingJoinPoint proceedingJoinPoint) throws Throwable { // ProceedingJoinPoint 이라는 매개변수를 가져온다.
        // proceedingJoinPoint.proceed(); 에서 .proceed();는 doFilter 같은 역할을 한다.

        Object[] args = proceedingJoinPoint.getArgs(); // getArgs 배열로 리턴되서 배열을 받을 수 있다.
        BeanPropertyBindingResult bindingResult = null;

        for(Object arg : args) {
            if(arg.getClass() == BeanPropertyBindingResult.class) {
                bindingResult = (BeanPropertyBindingResult) arg; // arg get 클래스가 이것이면 다운캐스팅 해서 대입함
                break;
            }
        }
